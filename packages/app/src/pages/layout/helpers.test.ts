...
describe("layout deep links", () => {
  ...
  test("ignores non-project deep links", () => {
    expect(parseDeepLink("chimera://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("chimera://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("chimera://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("chimera://open-project")).toBeUndefined()
    expect(parseDeepLink("chimera://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "chimera://open-project?directory=/a",
      "chimera://other?directory=/b",
      "chimera://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("chimera://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink("chimera://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("chimera://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("chimera://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "chimera://new-session?directory=/a",
      "chimera://open-project?directory=/b",
      "chimera://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __OPENCODE__: {
        deepLinks: ["chimera://open-project?directory=/a"],
      },
    } as unknown as Window & { __OPENCODE__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["chimera://open-project?directory=/a"])
