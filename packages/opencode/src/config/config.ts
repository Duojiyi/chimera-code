...
function globalConfigFile() {
  const candidates = ["chimera.jsonc", "chimera.json", "opencode.jsonc", "opencode.json", "config.json"].map((file) =>
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadGlobal = Effect.fnUntraced(function* (env?: Record<string, string>) {
      ...
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "chimera.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "chimera.jsonc"), env))
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        if (!Flag.OPENCODE_DISABLE_PROJECT_CONFIG) {
          for (const name of ["opencode", "chimera"]) {
        ...
        for (const dir of directories) {
          if (dir.endsWith(".opencode") || dir === Flag.OPENCODE_CONFIG_DIR) {
            for (const file of ["opencode.json", "opencode.jsonc", "chimera.json", "chimera.jsonc"]) {
