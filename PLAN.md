...
22. ✅ 二轮 1:1 精修（2026-08-12 晚）：
    - S6：账号卡（渐变神兽标头像/用户名持久化/退出登录=DELETE /auth）+
      密钥表列宽/复制图标/删除收 hover；头像组件 ChimeraAvatar 替换"多"字
    - S3：真表格（●点+mono id/协议 chip/上下文/默认 radio/状态开关），
      默认=置顶最近使用（composer 默认取值链真实写入），点击实测生效
    - S1：轮首回复头组件化（渐变标+Chimera+模型 id·时间）、元信息行
      「耗时 m:ss · tokens · $」等宽、发送按钮熔金、Enter/Esc 提示
    - S0：单列重排（标题左/搜索+新建右，弃项目栏）
    - S5：品牌头/管理员预置徽章/图标前缀输入框/密码可见切换/按钮图标
    - 对齐修复：连接成功、密钥切换、退出登录后 refreshProviders 即时生效
      （此前需手动刷新）；实测断开→零状态→重连→模型恢复全链路
    - CDP 截图在深色弹窗顶部有白带伪影，原生截屏证实真实 UI 正常
23. ✅ 三轮 1:1 精修（2026-08-12 夜）：
    - 默认深色（preload + 运行时双层，设计稿主视觉）
    - S1 任务清单卡：todoState 改为未完成任务空闲常驻（同步单测 14 pass），
      行尾状态标签（完成=青金/进行中=熔金/等待=灰），实测三任务一停二常驻
    - S1 状态栏上下文渐变条（最近请求 tokens ÷ 模型窗口，品牌渐变线落地；
      数据源修正为 session 服务 store）；输入区模型选择描边胶囊化
    - S4 命令面板：底部按键提示条 + 渐变小条、快捷键徽章等宽轻量化
    - S6 密钥表移除"本月用量"列（放弃网关统计接口依赖）
    - 审查面板深色验证（非 git 项目如实空态）
    - 发现网关超时会触发上游自动重试产生重复用户消息（服务端行为，
      时间线如实渲染，非 UI 问题）
24. ✅ 四轮 1:1 对照打磨（2026-08-12 深夜，对照设计稿逐像素）：
    - 排版密度：内容列 800→640px、正文 13.5px、用户消息深底无边框紧凑卡、
      隐藏用户消息元信息行（模型/时间已在 AI 回复头）
    - 输入区：熔金描边常亮 + focus 发光（设计稿视觉焦点）、发送钮熔金常亮、
      附件按钮描边化、模型描边胶囊
    - 活动行右侧耗时（ms/s 自适应，data-duration 纯 CSS 呈现）
    - 状态栏：会话变更统计 +N -N · N 处更改（session_diff 真实聚合）
    - 终版截图 design/final-s1-dark.png
25. ✅ 五轮打磨（2026-08-12 深夜，自审迭代 + 浅色复核 + i18n）：
    - 自审修复：任务清单复选框圆形化+状态语义色、正文段距 8px、
      用户消息圆角统一 10px、首页会话行尾相对时间、S3/S6 表行 hover、
      命令面板分组标题 mono 眉标
    - 浅色主题逐屏复核（S0/S1/S6）：语义变量自适应，无对比度问题
    - chimera 组件 i18n 键补全（chimera.* 约 90 键，en/zh 双包，
      中文值与原文案逐字一致），实测渲染无回归
26. ✅ 六轮打磨（2026-08-13 凌晨，网关直连 + 数据自动化 + 视觉三连）：
    - desktop 主进程网关代理（仅限网关域名 /api/），账号密码登录直连
      new-api 同步账号下全部密钥进本地管理器；Turnstile 感知报错（实测
      真实网关往返，站长关闭 Turnstile 后全链路即通）；插件 authorize
      同步真实化（CLI 场景）
    - 模型元数据自动识别：merge models.dev 官方目录（跨厂商按 ID 匹配），
      上下文实测区分 200k/1m，官方名/能力/日期同步；网关模型默认全可见
    - 移除美元费用显示（仅保留 tokens）；输入区与内容列同轴 640px；
      主操作按钮熔金化（neutral 变体全局)；S5 segmented 白底高对比选中态
    - 切换密钥/重连后模型列表即时刷新（refreshProviders 链路复验 ✓）
27. ✅ 七轮（2026-08-13 凌晨，设备授权流 + 真实像素验收）：
    - 设备授权流全链路（RFC 8628 最小实现）：网关侧三端点 + Go 渲染浏览器
      确认页（账号密码与 Turnstile 全程在浏览器）+ auth_flows 复用存储 +
      一次性消费防重放；本地隔离实例端到端冒烟发现并修复 SQLite 事务自锁；
      运维文档 new-api docs/chimera-desktop-auth.md
    - 客户端「设备授权 / API 密钥」双页签：设备码大字 + 自动开浏览器 +
      轮询自动完成 + 零令牌自动创建 Chimera Desktop 令牌；
      插件退役账号密码方式（零凭据原则）
    - Windows MCP 真实像素验收：S5 设备授权页 / S1 会话（内容 640 居中、
      输入同轴、无 CDP 伪影困扰）确认达标；S0/S4/S6 全屏复检达标；
      用户使用电脑期间改用 CDP 静默截图避免抢焦点
    - 待用户操作：部署 new-api（含设备流）后用管理员账号真跑一遍授权
28. ✅ 八轮（2026-08-13 清晨，全屏 1:1 实拍比对 + 入口路由修复）：
    - 方法论升级：窗口调为物理 1800x1125（= 逻辑 1440x900 与设计稿画板一致），
      DPI 感知 DWM 截屏（design/audit/capture.ps1），逐屏与 2x 设计稿并排核对
    - 功能 bug：连接入口路由缺口——「更多提供商 → Chimera」走了上游通用
      方法对话框而非 S5 定制页；修复为三处统一分流（选择器拦截 + 设置页
      connect 按连接状态分流 KeysDialog/ConnectDialog + chimera 置顶热门列表）
    - S0：搜索框透明细描边、新建会话金实心、会话行绿色状态点 +
      行尾「项目名 · 相对时间」mono 元信息（弃项目头像徽标）
    - S1：首条消息日期分隔线（今天/昨天/日期，居中发丝线）、placeholder
      按设计稿（构建期文案替换表）、发送按钮禁用态维持熔金 62%、
      状态栏「● 网关 {ms}ms」延迟探测（60s）+ 品牌版本 v0.1.0（BRAND.version）
    - S2：内容上移至 24cqh、副标题 i18n 化、卡片下快捷键提示行
    - S3/S6：表头黑条移除（透明+发丝分隔）、行透明化、行高 40px、
      开关改标准圆形滑动 toggle（上游样式在高优先级层，需 !important 覆盖）
    - S4：无框搜索行（贴顶+底部分隔线）、快捷键细边框徽标（推翻前轮
      "透明轻量"误判）、选中行熔金 9% 高亮、底部提示条 i18n 化
    - S5：logo 改深底金标 app 图标风格
    - 复验：typecheck ✓；重启后 vite 文案替换、网关延迟、日期分隔线、
      开关/表格样式全屏实拍逐项确认 ✓（design/audit/audit9-*.png）
29. ✅ 九轮（2026-08-13 清晨，用户红框反馈 + 主题体系调整）：
    - 主色去"屎黄"：根因是主题引擎将 palette #DEA54C 按色阶提亮为
      #F8BD63（yellow-500）；用 v2Overrides 钉死 v2-state-fg-warning
      （深色 #CE9740 沉琥珀 / 浅色 #9A681B 深金褐），全部金色控件联动
    - 默认浅色（preload + context 双处），左栏新增深浅快速切换按钮
      （sun/moon，i18n 化 tooltip）
    - S1 内容列 640→760（用户实拍指出两侧空白过大；设计稿卡宽按含右
      面板布局折算）；发送按钮改圆形；模型 pill 隐藏厂商图标改金点前缀
    - 模型表：协议列按模型家族推断（claude→Anthropic、gpt/o→OpenAI、
      gemini→Google，回落 OpenAI 兼容）；models.dev 展平改一方厂商优先
      （修复 sonnet-4-5 被第三方渠道 1M 值污染，官方 200k）；搜索框
      描边胶囊化；1m 大量出现经核实为官方真值（opus4-6+/sonnet4-6+ 1M）
    - S0 会话行补底部发丝分隔线；密钥页左对齐经 DOM 实测无偏差
      （463/463/464/463），用户红框疑点为截图裁切边缘误导
    - 复验：typecheck ✓；浅色 S0/S1/S3/S6 + 深色 S1 实拍确认；
      深色"文字变深"为缩略图视觉误判（放大与 DOM 均为正常白字）
30. ✅ 十轮（2026-08-13 早，"灰按钮"真因 + 留白）：
    - 发送按钮"灰色"真因终于定位：上游 icon-button primary 用双层
      background-image 渐变（白高光 + 实心灰）绘制按钮主体，属性级层叠里
      image 盖住 background-color——此前所有"金色化"只改了 color 一直被
      盖住。修复：显式接管 image 层（保留高光、清掉灰底）+ 满金 disabled
      （透明度/混色方案都会洗成脏灰驼色，已废弃）
    - 排查过程排除法记录：computed 金色 + CDP 合成像素灰 + 无
      filter/blend/伪元素/覆盖层 → background-image；中心像素采样曾
      误采箭头图标（按钮中心是图标不是背景）
    - 浅色主色 #9A681B → #A9761C（略暖亮，去酱油感）
    - 设置模型/密钥两 tab 内容区补 padding（px-9 pt-8 pb-10），
...
32. ⏳ 遗留（结构性，需专项决策）：顶栏面包屑（与多会话 tab 栏结构冲突）、
    ...
    跨行合并活动卡带步数 header（虚拟化行间无 DOM 兄弟关系）、
    命令面板行图标（command registry 无 icon 字段）、S0"进行中"独立分组、
    S3"添加自定义模型"按钮（保留搜索框，后端无对应功能）、
    CSS content 状态标签（活动/进行中/等待/完成）为中文硬编码（CSS 不可 i18n，
    如需多语言需转组件渲染）、CI 自动打包发布（Phase 3）、
    中转站上游身份透传（需网关侧渠道配置）、网关延迟波动与自动重试造成
    重复用户消息（服务端行为）
